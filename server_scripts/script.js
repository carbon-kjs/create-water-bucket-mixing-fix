ServerEvents.recipes((event) => {
  event.forEachRecipe(
    { input: "minecraft:water_bucket", type: "minecraft:crafting_shapeless" },
    (recipe) => {
      ...
      if (
        recipe.result === "create:dough" ||
        recipe.result === "create:wheat_dough"
      )
        return;

      // Add the old recipe's ingredients to a new mixing recipe
      recipe.id(`${recipe.getId()}_manual_only`);
      event.recipes.createMixing(recipe.result, new_ingredients);
    }
  );
});
